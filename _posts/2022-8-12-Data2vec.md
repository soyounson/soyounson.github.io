...
☺︎ What the data2vec is. ⇨ 1.Introduction

☺︎ Related work ⇨ 2. Related work
...
☺︎ Experimental setup + Results  ⇨ 4. Experimental setup + 5. Results 
  - Computer vision 
  - Speech processing 
  - Natural language processing (NLP)
  - Ablations

☺︎ Discussion  ⇨ 6. Discussion 
...
keywords 
중요한것은 여러 모다에서 사용가능한 것을 만들고자 하는데, 
하는 방법들은 각 모달마다 바꿔줘야 한다는 특징이 존재함. 

~~~~~~~~~~~~~~~~~~~~~

~~~~~~~~~~~~~~~~~~~~~

### ☺︎ What the data2vec is [2,3].

Data2vec은 self-supervised learning (자기지도학습)으로 multimodality (speech, image, text)에서 적용 가능한 framework로 . 이는 기존에 supervised learning (지도학습)으로 특정 단일 modality에 집중해서 각 modality별로 학습 방법을 다르게 했던 것에 비해 좀 더 일반적인 방법으로 확장된 형태이다 
...
하지만 학습 알고리즘은 통합적이지만 여전히 representation은 각 modality의 양식에 대해 개별적으로 학습한다.
...
하지만 data2vec의 경우도 input encoder와 masking이 modality specific하다는 한계점이 존재한다. 이는 추후 이 과정들도 모두 modality independent 하게 바꾸는 방향으로 나아가야 한다고 생각한다. 
...
3. [blog : Data2Vec Review](https://baekyeongmin.github.io/paper-review/data2vec/)
4. [blog : 초간단 논문리뷰, data2vec](https://everyday-deeplearning.tistory.com/m/entry/data2vec-A-General-Framework-for-Self-supervised-Learning-in-Speech-Vision-and-Language)
