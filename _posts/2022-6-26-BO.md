...
Hyperparameters란 learning 시작하기 전에 미리 정하는 값으로, 모델이 learning하는 것이 아니라 training전에 미리 정하는 값으로 최적의 모델을 구하기 위해서 tuning을 해줄 필요가 있다. 
...
왜 Hyper-라는 말을 붙였는지에 대해서 좀 더 자세히 살펴보자면 
...
좀 더 deep dive해서 확인하면, 최적의 parameter을 배우기위해 사용되는 (미리 정해서 넣어주는) 값이 hyperparamter 이다. 그래서 초변수? 라고 볼 수 있다. 
...
방법들에 대한 것은 out of our scope이라서 더 이상 다루지 않고, 바로 Bayesian optimization으로 넘어가도록 한다. 
